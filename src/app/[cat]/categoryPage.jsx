...
const CategoryPageMain = ({ params }) => {
  ...
  const currentCategoryRef = useRef(cat);
  
  // State to show skeleton during category transition
  const [isCategoryTransition, setIsCategoryTransition] = useState(false);
  ...
  useEffect(() => {
    if (initialSearchDone.current && !isCategoryTransition) return;
    ...
    if (isCategoryTransition) {
      setTimeout(() => {
        setIsCategoryTransition(false);
      }, 100);
    }
  }, [cat, limit, dispatch, sort, isCategoryTransition]);
  ...
  }, [selectedFilters, cat, limit, debouncedSearch, sort]);
  ...
  const showSkeleton = isLoading || isCategoryTransition;
