...
"use client"
import { getUserByMob, send_sms_through_backend, set_checkout_authentication_status, signin } from "@/redux/athentication/Athentication";
...
const OtpPage = ({ onVerify, onResend }) => {
  ...
  useEffect(() => {
    if (buttonPress === true) {
      ...
      if (allEmpty === false) {
        ...
        // setOtpValuesErrors("enter valid otp...")
      }
    }
  ...
  const handleKeyDown = (event, index, refs) => {
        ...
        <div className="relative z-10 max-w-xs sm:max-w-lg w-full">
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-2xl px-8 py-5 border border-gray-100">
            <div className="text-center sm:mb-8 mb-4 ">
              ...
              <h2 className="sm:text-3xl text-xs font-bold text-gray-800 sm:mb-2 mb-0">Verify OTP</h2>
              <p className="text-gray-600">
                We've sent a 6-digit code to
                <br />
                <span className="font-semibold text-[#2f415d]">+91 {mob}</span>
              </p>
            </div>
            ...
            <form className="sm:space-y-6 space-y-3 ">
              ...
              <div className="flex justify-center sm:space-x-3 space-x-2">
                {otpValues.map((digit, index) => (
                  <input
                    ...
                    value={digit}
                    onChange={(e) =>
                      handleOtpChange(index, e.target.value.replace(/\D/g, ""), inputRefsDesktop)
                    }
                    onKeyDown={(e) => handleKeyDown(e, index, inputRefsDesktop)}
                    onPaste={handlePaste}
                    className={`sm:w-12 sm:h-12 w-10 h-10 text-center text-xl font-bold rounded-xl transition-all duration-300 ${otpValuesErrors.includes(index)
                  ...
                  // className="sm:w-12 sm:h-12 w-10 h-10 text-center text-xl font-bold border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2f415d]/20 focus:border-[#2f415d] transition-all duration-300 bg-gray-50/50 hover:bg-white"
                  />
                ))}
              </div>
              {logerror && (
              ...
              <button
                ...
                className={`group w-full py-4 px-8 rounded-xl font-semibold text-lg shadow-lg transition-all duration-300 transform ${!isVerifying
