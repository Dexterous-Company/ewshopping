...
const BannerGrid = () => {
  const { cardPromotions = [], loading, error } = useSelector((state) => state.cPromotion);
  ...
  const handleClick = (e, promotion) => {
    e.preventDefault();
    if (promotion) {
      router.push(`/searchresults?subCategory=${encodeURIComponent(promotion.subCategory)}`);
    }
  };
          ...
          {cardPromotions.map((promotion) => {
            ...
            return (
              <div
                ...
                className="relative min-w-[200px] sm:min-w-[400px] cursor-pointer aspect-[2/1] group flex-shrink-0"
              ...
              >
                ...
                >
                  <Image
                    ...
                    quality={70}
                  />
                  <div className="absolute inset-0 flex flex-col items-start justify-center p-6 text-left z-10 text-black">
                    <h3 className="text-sm sm:text-lg md:text-2xl font-bold mb-1 sm:mb-2 transition-all duration-500 group-hover:translate-x-[-10px]">
                      {promotion.mainTite}
                    </h3>
                    <p className="text-xs sm:text-sm md:text-base mb-2 sm:mb-4 transition-all duration-500 group-hover:translate-x-[-5px]">
                      {promotion.title}
                    </p>
