...
const ManageAddress = () => {
  const { user_address, current_address, loginData } = useSelector((state) => state.Athentication);
  ...
  return (
    <div className="sm:px-4 px-2 relative py-2 w-full sm:h-[70vh] sm:mb-0 mb-10 overflow-y-auto">
      ...
      <div className="sm:hidden block">
        <div
          className="w-full shadow-md px-4 border border-gray-300 py-3 flex items-center gap-3 hover:bg-gray-100 cursor-pointer"
          onClick={() => router.push("/accounts/address/addaddress")}
        >
          <FaPlus className="text-[#2f415d]" />
          <span className="text-[#2f415d] font-semibold text-sm">
            Add New Address
      ...
      <div className="sm:mt-5 mt-3 px-4 py-3 sm:shadow-md shadow-sm w-full flex flex-col gap-4">
        {user_address.length > 0 ? user_address.map((address, index) => (
          <div
            key={address._id}
            onClick={() => handleSelectAddress(index)}
            className={`flex justify-between items-start border p-4 rounded-md transition-all duration-150 ${select === index
              ...
              <div className="flex gap-2">
                ...
                <div className="block sm:hidden">
                  <button
                    ...
                    onClick={() =>
                      router.push(
                        `/accounts/address/editaddress?id=${address._id}`
                      )
                    }
                  >
                    Edit
