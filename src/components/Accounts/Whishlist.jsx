  ...
  const handleDelete = async () => {
    if (whislistObject) {
      await dispatch(
        removeFromWishlistServer({
          userId: loginData._id,
          ProductId: whislistObject.ProductId,
        ...
        {wishlistItems.length > 0 ? (
          <>
            {/* {wishlistItems.map((item, index) => (
              <div
                key={index}
                className={`flex flex-row flex-wrap sm:flex-col justify-between ${
                  index && "hover:bg-gray-300"
                } sm:w-full items-start border  border-gray-300 sm:p-4 rounded-md w-[40vw] transition-all duration-150 cursor-pointer hover:shadow-md hover:bg-gray-50`}
              >
                <div className="flex flex-col sm:flex-row sm:gap-3 items-start w-full">
                  ...
                  <div className="flex sm:max-w-[20vw] w-full sm:mb-0 mb-2 sm:px-0 px-3 flex-col gap-4 sm:justify-between items-center sm:items-end">
                    <button
                      className={`whitespace-nowrap w-full sm:w-1/2 px-4 py-2 sm:py-1 font-semibold text-sm cursor-pointer ${
                        CartItems.some(
                          (cartItem) =>
                    ...
                    <span
                      className="text-gray-600 hidden sm:block font-medium text-2xl"
            ...
            {/* {wishlistItems.map((item, index) => {
              // Find the product details from wishlistProducts
              const productDetails = wishlistProducts.find(
                (prod) => prod.productSlug === item.productSlug
              );
              const availableStock = productDetails?.availablestock ?? 0;
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:gap-3 items-start w-full">
                    <div className="flex relative flex-row justify-between items-center">
                      ...
                      />
                      {availableStock <= 0 && (
                        <span className="absolute top-1 left-1 bg-blue-950 text-white text-xs px-2 py-1 rounded">
                          Out of Stock
                        </span>
                      )}
                    </div>
                    <div className="flex flex-col sm:py-0 py-2 sm:px-0 px-3 w-full ">
                      ...
                      <button
                        className={`px-4 py-2 mt-2 font-semibold text-sm rounded ${
                          availableStock <= 0
                            ? "bg-gray-400 text-white cursor-not-allowed"
                            : "bg-[#2f415d] text-white"
                        }`}
                        onClick={() =>
                          availableStock > 0 && handleAddTOCart(item)
                        }
                      >
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:gap-3 items-start w-full">
                    <div className="flex relative flex-row justify-between items-center">
                      ...
                      <span className="absolute text-[#e96184] bg-white text-xs text-center bottom-0">
                        {availableStock <= 0 ? "Out of Stock" : item.avilable}
                      </span>
                    </div>
                    <div className="flex flex-col sm:py-0 py-2 sm:px-0 px-3 w-full ">
                      <Link href={`/product/${item.productSlug}`}>
          ...
          <div className="flex flex-col items-center justify-center w-full min-h-[200px] text-center py-10">
            ...
            <Link href={"/"}>
              <button className="px-4 py-2 bg-[#2f415d] text-white rounded-md">
                Continue Shopping
              </button>
            </Link>
          </div>
